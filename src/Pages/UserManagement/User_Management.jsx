...
const User_Management = () => {
  ...
  useEffect(() => {
    axios
      .get(`${constantApi.baseUrl}/user_management/list`)
      .then((res) => {
        console.log("response is ------", res);
        setUserManagementData(res.data.data);
      })
  ...
  const handleEditActivity = (id) => {
    const user_management = userManagementData.find((data) => data.id === id);
    navigate("/edit_user_management", { state: { user_management } });
  };
  const handleViewActivity = (id) => {
    const user_management = userManagementData.find((user) => user.id === id);
    navigate("/view_user_management", { state: { user_management } });
  };
  const deleteModule = (id) => {
    axios
      .delete(`${constantApi.baseUrl}/user_management/${id}`)
      .then(() =>
        setUserManagementData((prev) => prev.filter((data) => data.id !== id))
      )
      .catch((err) => alert("Failed to delete the module."));
  };

  const filteredModules = userManagementData.filter((data) =>
          ...
          <tbody>
            {filteredModules.map((data) => (
              <tr key={data.module_id} className="border-b">
                <td className="p-4 leading-tight">{data.first_name}</td>
                <td className="p-4 leading-tight">{data.last_name}</td>
                <td className="p-4 leading-tight">{data.email}</td>
                <td className="p-4 leading-tight">{data.phone_number}</td>
                <td className="p-4 leading-tight">{data.organisation}</td>
                <td className="p-4 leading-tight">{data.location}</td>
